...
const checkZRead = async (previewPayload) => {
  const [date] = previewPayload.transactionDate.split(' ');

  const [, startTime] = moment(previewPayload.transactionDate)
    .startOf('day')
    .format('YYYY-MM-DD HH:mm:ss')
    .split(' ');
  const [, endTime] = moment(previewPayload.transactionDate)
    .endOf('day')
    .format('YYYY-MM-DD HH:mm:ss')
    .split(' ');

  const zReadExist = await Preview.findOne({
    transactionDate: {
...
const printZReadService = async (printData) => {
  ...
  printer.newLine();
  printer.println(
    `SHIFT CLOSING of ${cashier.lastname.toUpperCase()}, ${cashier.firstname.toUpperCase()} - ${
      cashier.id
    }`
  );
  ...
  printer.println(`Transaction date: ${moment(initialFund.cash_date).format('MM/DD/YYYY')}`);
  ...
  if (payments.nonCash.giftCards.summary.count > 0) {
    ...
    payments.nonCash.giftCards.GC_ITEMS_METHODS.forEach((gift) => {
      printer.leftRight(
        `${gift.method.toUpperCase()} (${gift.count})`,
