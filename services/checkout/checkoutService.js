...
exports.printCheckoutReceipt = async (printData, {siNumber, txnNumber}) => {
  ...
  cart.confirmOrders.forEach((order) => {
    ...
    printer.leftRight(`SI No.: ${siNumber}`, 'PHP');
  ...
  if (cart.payments.filter((x) => x.value === 'rmes').length > 0) {
    ...
    printer.println(
      `Orig Trans Date: ${
        origDate.getMonth() + 1 > 9 ? origDate.getMonth() + 1 : `0${origDate.getMonth() + 1}`
      }/${
...
exports.updateOrders = async (order, session, { txnNumber, siNumber }) => {
  const { orderId, products, transactionDate, total } = order;
...
exports.createPosDiscountOrderLogs = async (discountOrders, session) => {
  for (const discountOrder of discountOrders) {
    const {
      discountLogId,
      discount,
      amount,
...
exports.createPosDiscountItemLogs = async (discountItems, session, { txnNumber }) => {
...
exports.createPosSCPWDReports = async (scPWDReports, session, { txnNumber }) => {
  for (const scPWDReport of scPWDReports) {
    const {
      scPwdReportId,
      firstname,
      lastname,
...
exports.createPaymentLogs = async ({totalPayment, cashChange}, payments, session, { txnNumber }) => {
  let totalCartPayment = totalPayment
  for (const payment of payments) {
    let paymentAmount = payment.amount;

    if (payment.customPaymentKey === 'cash' && payment.amount > totalCartPayment) {
      paymentAmount -= cashChange;
    }

    if (paymentAmount >= totalCartPayment) {
      paymentAmount = totalCartPayment
    }

    totalCartPayment -= paymentAmount;
    ...
    await Order.findOneAndUpdate(
      { txnNumber: txnNumber },
      { $push: { paymentMethods: newPaymentLog[0]._id } },
      { session }
    );
  }
};

exports.createScPwdDiscountLogs = async (scPwdDiscounts, session, { txnNumber }) => {
  for (const scPwdDiscount of scPwdDiscounts) {
    const {
      scPwdReportId,
      firstname,
      lastname,
...
exports.createPosDiscountTransactionLogs = async (discountTransactions, session, { txnNumber }) => {
  for (const discountTransaction of discountTransactions) {
    const {
      discountLogId,
      discount,
      amount,
...
exports.createPromoCodeLogs = async (promoCodeLogs, session) => {
  for (const promoCodeLog of promoCodeLogs) {
    const { promoCodeLogId, promoCodeId, promoType, value, discountType } = promoCodeLog;

    await PromoCodeLog.create([
      {
        promoCodeLogId,
