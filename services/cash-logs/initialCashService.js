...
const checkCashLog = async (initialCashDate, initialCash) => {
  const initialCashLogCount = await CashLog.aggregate([
    {
      $addFields: {
        cashDateString: {
          $dateToString: {
    ...
    {
      $match: {
        cashDateString: initialCashDate,
  ...
  return initialCashLogCount.length > 0;
};

const printInitialCash = async (printData) => {
